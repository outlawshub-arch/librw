...
installDepthTexture(void)
{
	...
	// NB: with in-game MSAA (Antialiasing) ON we skip here, so the depth-based water
	// transparency + foam are unavailable until AA is turned off. This is a hard D3D9
	// limitation (a multisampled depth surface can't be bound as a texture).

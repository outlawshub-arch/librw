...
void *im2dOverridePS;
// [LOCAL PORT: EX_PORT_WATERREF] Im3D shader-override hooks. Set both, do the Im3D
// draw, unset. Lets an external caller (e.g. planar water reflection) replace the
// default VS/PS around an Im3D draw batch without patching the pipeline itself.
void *im3dOverrideVS;
void *im3dOverridePS;
// [LOCAL PORT: EX_PORT_WATERREF] Force sampler 0 to a specific texture right before
// each Im3D draw. The rw state cache can go out of sync when direct d3d::setTexture
// calls happen during atomic pipe callbacks (Im3D flat water passes see stage 0
// clobbered to white); binding this directly here avoids the cache short-circuit.
Texture *im3dOverrideTex0;
